...
void SceneManager::onAwake()
{
  ...
  dontDestroyOnLoad(watermark.get());
...
void SceneManager::onGui()
{
  ...
  {
    Gui::drawTexture(Rect(Screen::getWidth() / 2 - watermark->getWidth() / 2, 40, watermark->getWidth(), watermark->getHeight()), watermark.get());
  }
  else
  {
    Gui::drawTexture(Rect(0, Screen::getHeight() - watermark->getHeight() / 2.0f - 25, watermark->getWidth() / 2.0f, watermark->getHeight() / 2.0f), watermark.get());
  }
}
